...
REPLICATE_MODELS = {
    ...
    #     'Flan T5': {
    #     'replicate_id': 'replicate/flan-t5-xl:1457f256622cd45415aa70c02105a917b39dc96e58601d7c2df5a30d2e3092e7',
    #     'description': 'Stable Diffusion, a latent text-to-image diffusion model capable of generating photo-realistic images given any text input.',
    #     'input_type': 'text',
    #     'output_type': 'photo',
    # },
...
async def replicate_set_select_param(param_name: str, value: str, botnav: TeleBotNav, message: Message):
    ...
    await botnav.bot.send_message(message.chat.id, f"Параметр был установлен на: {value}")
...
async def replicate_choose_param(model_name_param_name: str, botnav: TeleBotNav, message: Message):
    ...
    if param['type'] == 'int':
        text = "Пожалуйста, введите целое значение "
        if param.get('description'):
            text += f"({param['description']}) "
        if param.get('min'):
            text += f"больше чем {param['min']} "
        if param.get('max'):
            text += f"меньше чем{param['max']} "
        if param.get('default'):
            text += f"или оставьте пустым для значения по умолчанию ➡️ ({param['default']})"
...
async def replicate_print_params_buttons(botnav: TeleBotNav, message: Message):
    ...
    if model.get('available_params'):
        ...
        await botnav.print_buttons(
            message.chat.id,
            buttons,
            text="Если вы хотите установить дополнительные параметры, используйте кнопки:",
            row_width=2,
        )
...
async def replicate_choose_model(model_name: str, botnav: TeleBotNav, message: Message) -> None:
    ...
    await botnav.bot.send_message(message.chat.id, "Модель сохранила параметр.")# + REPLICATE_MODELS[model_name]['description']
...
async def start_replicate(botnav: TeleBotNav, message: Message) -> None:
    await botnav.print_buttons(
        ...
        row_width=2,
        text=f"▫️ Выберите модель:\n \n*Напишите промт на англ языке. "
    )

    botnav.wipe_commands(message, preserve=['start'])
    botnav.add_command(message, 'replicate_models', '🖼 Replicate модели', start_replicate)

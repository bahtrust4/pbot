...
async def yt_set_format(format: str, botnav: TeleBotNav, message: Message) -> None:
    ...
    await botnav.bot.send_message(message.chat.id, f'Формат установлен на {format}')
...
async def yt_format(botnav: TeleBotNav, message: Message) -> None:
    await botnav.print_buttons(
        message.chat.id,
        {
            'Aудиo+Видео': functools.partial(yt_set_format, 'mp4'),
            'Только аудиo': functools.partial(yt_set_format, 'm4a'),
        },
...
async def youtube_dl_message_handler(botnav: TeleBotNav, message: Message) -> None:
    ...
    try:
        ...
        try:
            ...
            if os.stat(filename).st_size > MAX_FILE_SIZE:
                if not config.YT_DL_DIR:
                    await botnav.bot.send_message(message.chat.id, "Файл слишком большой, повторите попытку позже.")
                    return

                url = make_file_available_by_url(filename)

                await botnav.bot.send_message(message.chat.id, f"Файл слишком большой, скачайте его с {url}")
                return
        ...
        with open(filename, 'rb') as file:
            await botnav.bot.send_document(message.chat.id, file)
        logger.exception(exc)

   
async def start_youtube_dl(botnav: TeleBotNav, message: Message) -> None:
    photo = open(os.path.join("C:\\Users\\User\\Documents\\GitHub\\Bott\\photos", "photo.jpg"), "rb").read()
    await botnav.print_buttons(
        message.chat.id,
        {
            'Format': yt_format,
        },
        f'Приветствуем на Youtube Dl.\n \nПришлите мне URL(как на фото) ссылки для скачивания!\n \nИ выберите требуемый формат(видео/аудио):',
        row_width=2
    )
    ...
    # await botnav.bot.send_message(message.chat.id, ' и пришлите мне URL ссылки для скачивания!')
